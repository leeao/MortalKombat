...
class rClump(object):
        ...
        def readClump(self):     
            ...
            if geometryCount:
                    ...
                    geometryList = rGeometryList(self.bs,geometryCount,vertMatList,frameList.hAnimBoneMap)      # Does not contain duplicate bones
                    #geometryList = rGeometryList(self.bs,geometryCount,vertMatList,frameList.hSkinBoneIDList)  # Contains duplicate bones                    
                    ...
                    if LoadMKAnimation:
                        ...
                        for mkaFile in mkaFiles:
                            ...
                            if animData:                        
                                anims.append(LoadAnims(animData, mkaName, skinBones,frameList.hAnimBoneIDList,frameList.hSkinBoneIDList))
                        if anims:
                            mdl.setAnims(anims)
                ...
                if self.hasHAnim > 0:
                            ...
                            if hAnimBoneID == animBoneID:
                                ...
                                if self.hasBoneName == 1:
                                    ...
                                    if GlobalBoneNames.get(str(self.hAnimBoneIDList[index1]& 0xffff)  ):
                                        bname = "_" + GlobalBoneNames[str(self.hAnimBoneIDList[index1] & 0xffff) ]
                                    #if (self.hAnimBoneIDList[index1] & 0xffff) != hAnimBoneID:
                                    #    if GlobalBoneNames.get(str(hAnimBoneID)  ):
                                    #        print("'%d':\"%s_copy\"," % ((self.hAnimBoneIDList[index1] & 0xffff),(GlobalBoneNames[str(hAnimBoneID)]))  )
                                    newSknBone.boneName = "Bone" + str(SkinBoneID)+"_"+ str(index1) +"_"+str(hAnimBoneID)+ "_" + str(self.hAnimBoneIDList[index1]&0xffff)+ "_" + str(self.hAnimBoneIDList[index1] >> 16)+ bname
                    ...
                    for i in range(len(skinBoneInfoList)):
                        ...
                        if sknBne.listParentID != -1:
                            skinBoneInfoList[i].skinBoneParentID = self.boneInfoList[sknBne.listParentID].skinBoneID
                                ...
                                if curBoneAnimBoneID == self.hAnimBoneIDList[j] & 0xffff:
                                    ...
                                    if self.hasBoneName == 1:
                                        ...
                                        if GlobalBoneNames.get(str(curBoneAnimBoneID)  ):
                                            boneData.boneName = "Bone" + str(curBoneAnimBoneID)+ "_" + GlobalBoneNames[str(curBoneAnimBoneID) ]
                                        else:
                                            boneData.boneName = "Bone" + str(curBoneAnimBoneID)
...
class rSkin(object):
        ...
        def readSkin(self):
                if self.nativeFlag == 0:
                        ...
                        self.usedBoneIndexList=[]
                        for i in range(boneCount):
                                if self.version < 0x37000 and maxNumWeights == 0:
                                    flag = self.bs.readInt()
        ...
        def rGeometryStruct(self):
                ...
                if nativeFlags == 1 and geoStruct.chunkSize > 40:   
                    if isMKPS2:
                        ...
                        MKPS2NativeDataPLG = rMKPS2NativeDataPLG(self.bs,matList,binMeshPLG,self.vertMat,rMatList.MKMaterialSkinBonePalette,MKSkinFlag,self.hSkinBoneIDList,rMatList.useBonePalette,skinPlgBoneMap,rMatList.MKMaterialList)
                    ...
                    if isMKPSP:
                        ...
                        MKPSPNativeDataPLG = rMKPSPNativeDataPLG(self.bs,matList,binMeshPLG,self.vertMat,MKSkinFlag,self.hSkinBoneIDList,sphereXYZ,skin)
